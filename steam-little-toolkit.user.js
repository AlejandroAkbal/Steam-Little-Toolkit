...
(function () {
    ...
    if (!alreadyRun) {
        ...
        alert("First time settings are applied");

        // User Settings
        GM_setValue("useMenu", true);
        GM_setValue("bypassLinkFilter", false);
        GM_setValue("bypassAgeCheck", false);
        GM_setValue("removeOwned", true);
        GM_setValue("removeIgnored", false);
        GM_setValue("removeOwnedRecentlyUpdatedList", true);
        GM_setValue("removeIgnoredRecentlyUpdatedList", true);
        GM_setValue("removeLiveStreams", true);

    }

    //Reflection for loops
    let settings = {
        useMenu: GM_getValue("useMenu"),
        bypassLinkFilter: GM_getValue("bypassLinkFilter"),
        bypassAgeCheck: GM_getValue("bypassAgeCheck"),
        removeOwned: GM_getValue("removeOwned"),
        removeIgnored: GM_getValue("removeIgnored"),
        removeOwnedRecentlyUpdatedList: GM_getValue("removeOwnedRecentlyUpdatedList"),
        removeIgnoredRecentlyUpdatedList: GM_getValue("removeIgnoredRecentlyUpdatedList"),
        removeLiveStreams: GM_getValue("removeLiveStreams")
    }
    ...
    async function start() {
        ...
        if (GM_getValue("bypassLinkFilter") === true && urlContains('linkfilter') > -1) {
        ...
        if (GM_getValue("bypassAgeCheck") === true && urlContains('agecheck') > -1) {
        ...
        /* ----- Category: Recently updated ----- */
            ...
            if (GM_getValue("removeOwnedRecentlyUpdatedList") === true) {
                removeElements(document.querySelectorAll(".ds_owned"));
                console.log('Removed recently updated owned content');
            }

            if (GM_getValue("removeIgnoredRecentlyUpdatedList") === true) {
                removeElements(document.querySelectorAll(".ds_ignored"));
            ...
            Object.entries(settings).forEach(([settingName, value]) => {
                ...
                document.getElementById(idSettingName).addEventListener('change', function () {
                    ...
                    if (this.checked) {
                        ...
                        GM_setValue(`"${settingName}"`, true);
                        alert(GM_getValue(`"${settingName}"`));
